...
import { decodeBarcode, type Pixels } from '../src/qr/barcode';

/*
 * O writer 1D não vem neste build do @zxing/library, então geramos um Code 39
 * à mão (simbologia simples e determinística) para testar o decode de verdade.
...
describe('decodeBarcode', () => {
  it('lê um Code 39 gerado (round-trip)', () => {
    expect(decodeBarcode(code39('TEST-123'), true)).toBe('TEST-123');
  });

  it('lê outro Code 39 (letras e dígitos)', () => {
    expect(decodeBarcode(code39('QRUTILS42'), true)).toBe('QRUTILS42');
  });

  it('lê um código só com números', () => {
    expect(decodeBarcode(code39('1234567890'), true)).toBe('1234567890');
  });

  it('devolve null para uma imagem em branco (sem código)', () => {
    const width = 200, height = 80;
    const data = new Uint8ClampedArray(width * height * 4).fill(255);
    expect(decodeBarcode({ data, width, height }, true)).toBeNull();
  });

  it('não confunde ruído leve com código (retorna null)', () => {
